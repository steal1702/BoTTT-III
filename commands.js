...
exports.commands =
{
	...
	// Tour helper commands
	tour: "tour",
	tour: function(arg, by, room) {
		let arglist = arg.split(',');
		if(!arglist[0]) {
			this.say(room, "Please specify a tournament tier. **Command Usage**: ``.tour [tier], [cap]``. Tournament CAP is optional parameter. **List of Tour tiers**: ``.tour help``");
			return;
		}
	...
	start: function(arg, by, room) {
		let startTimer;
		if(!arg) {
			this.say(room, "Please specify tour timer value. **Command Usage**: ``.start [time in minutes]``");
			return;
		}
